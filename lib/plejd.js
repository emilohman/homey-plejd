...
class PlejdCommands {
  ...
  stateParse(data) {
    ...
    if (data.length === 10 || data.length === 20) {
      ...
      messages.forEach((message) => {
        ...
        const dim = dimFull >> 8;
        const coverPosition = this.parseCoverPosition(dimFull);
  ...
  notificationParse(data) {
    ...
    if (command === COMMAND_TIME_UPDATE) {
      ...
      dim = dimFull >> 8;
      coverPosition = this.parseCoverPosition(dimFull);
  ...
  parseCoverPosition(dimFull) {
    const raw = (dimFull & COVER_POSITION_PARSE_MAX) / COVER_POSITION_PARSE_MAX;
