...
const COVER_POSITION_MAX = 0xffff;
...
class PlejdCommands {
  ...
  coverSetPosition(id, normalizedPosition) {
    ...
    const encodedPosition = Math.round(position * COVER_POSITION_MAX);
    const low = encodedPosition & 0xff;
    const high = (encodedPosition >> 8) & 0xff;
    const payload = Buffer.from(
      `${id.toString(16).padStart(2, '0')}0110042003080701${low.toString(16).padStart(2, '0')}${high.toString(16).padStart(2, '0')}`,
      'hex',
    );
  ...
  stateParse(data) {
    ...
    if (data.length === 10 || data.length === 20) {
      ...
      messages.forEach((message) => {
        ...
        const dim = dimFull >> 8;
        const coverPosition = dimFull / COVER_POSITION_MAX;
  ...
  notificationParse(data) {
    ...
    if (command === COMMAND_TIME_UPDATE) {
      ...
      dim = dimFull >> 8;
      coverPosition = dimFull / COVER_POSITION_MAX;
