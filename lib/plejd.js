...
class PlejdCommands {
  ...
  thermostatSetTargetTemperature(id, temperature) {
    ...
    const payload = Buffer.from(`${(id).toString(16).padStart(2, '0')}0110045c${low.toString(16).padStart(2, '0')}${high.toString(16).padStart(2, '0')}`, 'hex');

    return encodeDecode(this.cryptokey, this.address, payload);
  }

  thermostatSetMode(id, mode) {
    const payload = Buffer.from(`${(id).toString(16).padStart(2, '0')}0110045f${(mode).toString(16).padStart(2, '0')}`, 'hex');
  ...
  notificationParse(data) {
    ...
    if (command === COMMAND_TIME_UPDATE) {
      ...
      if (decodedData.length >= 8) {
        const thermostatData = this.parseThermostatState(state, decodedData.readUInt8(6), decodedData.readUInt8(7), decodedData.length > 8 ? decodedData.readUInt8(8) : null);
    ...
    } else if (command === COMMAND_TRM_PWM_DUTY || command === COMMAND_TRM_RESET_OPERATING_MODE) {
      cmd = 'thermostat';
      ...
      const mode = decodedData.readUInt8(6);
      ...
      const data = decodedData.length > 8 ? decodedData.toString('hex', 7, 9) : '';
      cmd = 'state';

      // 00 01 10 00 16 0f 01 2d 0a

      this.logger.log(`Button id: ${buttonId} inputButton: ${inputButton} data: ${data}`);
      ...
      this.logger.log('Scene triggered ' + id + ' ' + state + ' ' + decodedData.toString('hex'));
    } else {
      this.logger.log('Unknown command: ' + id + ' ' + decodedData.toString('hex'));
  ...
  parseThermostatState(stateBit, payload0, payload1, payload2) {
    const stateRaw = ((stateBit & 0x01) << 16) | ((payload0 & 0xff) << 8) | (payload1 & 0xff);
    ...
    const heating = payload2 === null || payload2 === undefined ? null : Boolean(payload2 & 0x80);
  ...
  timeParse(data) {
    ...
    try {
      if ((decodedData.toString('hex', 0, 1) === '01' || decodedData.toString('hex', 3, 5) === '001b') && decodedData.length > 5) {
        const now = this._getUTCTime();
