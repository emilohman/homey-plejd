...
class PlejdApi {
  ...
  async login() {
    ...
    try {
      const response = await instance.post(API_LOGIN_URL, {
  ...
  async getSites() {
    ...
    try {
      ...
      logger(
        `error: unable to retrieve the crypto key. error: ${error} (code: ${error.response.status})`,
      );
      return Promise.resolve(false);
    }
  }
  ...
  async getSite(siteId) {
    ...
    try {
      ...
      return Promise.reject(
        new Error(`unable to retrieve the crypto key. error: ${error}`),
      );
    }
  }
  ...
  getDevices(filterType) {
    ...
    for (let i = 0; i < site.devices.length; i++) {
      ...
      let { name, type, dimmable, colorTemp } = this._getDeviceType(
        plejdDevice.hardwareId,
      );

      if (this._isThermostat(device, settings)) {
        name = 'TRM-01';
        type = 'thermostat';
        dimmable = false;
      }

      if (this._supportsColorTemperature(device)) {
        colorTemp = true;
      }
      ...
      if (filterType === type) {
        ...
        (type === 'light' || type === 'socket')
      ) {
        devices.push(newDevice);
      } else if (filterType === 'motion' && type === 'motion') {
        devices.push(newDevice);
      }
    }
  ...
  _isThermostat(device, settings) {
    const traits = Number(device && device.traits);

    if (
      !Number.isNaN(traits) &&
      ((traits & TRAITS.CLIMATE) === TRAITS.CLIMATE ||
        (traits & TRAITS.CLIMATE_PWM) === TRAITS.CLIMATE_PWM)
    ) {
      return true;
    }

    return Boolean(settings && settings.climateSettings);
  }

  _supportsColorTemperature(device) {
    const traits = Number(device && device.traits);

    return !Number.isNaN(traits) && (traits & TRAITS.TEMP) === TRAITS.TEMP;
  }
  ...
  _getDeviceType(hardwareId) {
    switch (parseInt(hardwareId, 10)) {
      ...
      case 3:
        return { name: 'CTR-01', type: 'light', dimmable: true };
      case 4:
      ...
      case 12:
        return { name: 'DAL-01', type: 'light', dimmable: true, colorTemp: true };
      case 13:
      ...
      case 20:
        return { name: 'SPR-01', type: 'socket', dimmable: false };
      case 36:
        return { name: 'LED-75', type: 'light', dimmable: true, colorTemp: true };
      case 70:
        return { name: 'WMS-01', type: 'motion', dimmable: false };
      case 103:
        return { name: 'OUT-01', type: 'light', dimmable: true, colorTemp: true };
      case 135:
        return { name: 'OUT-02', type: 'light', dimmable: true, colorTemp: true };
      case 167:
        return { name: 'DWN-01', type: 'light', dimmable: true, colorTemp: true };
      case 199:
        return { name: 'DWN-02', type: 'light', dimmable: true, colorTemp: true };
      default:
        return { name: '-unknown-', type: 'light', dimmable: true };
    }
  }
